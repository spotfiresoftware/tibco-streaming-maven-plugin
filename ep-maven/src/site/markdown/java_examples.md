...
4. Uses [start-nodes](./start-nodes-mojo.html) to start a test cluster.  
...
5. Uses [test-java-fragment](./test-java-fragment-mojo.html) to launch
    junit on the cluster and report the test results.  Should the test cases
    fail then no further processing occurs.
    
6. Uses [stop-nodes](./stop-nodes-mojo.html) to stop and remove the test 
    nodes
  
7. Uses [package-java-fragment](./package-java-fragment-mojo.html) to create
