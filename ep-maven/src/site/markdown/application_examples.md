...
2. Uses [package-application](./package-application-mojo.html) to create
a application archive zip file in the build directory (by default, set to 
target) and attaches it to the build.

3. Uses [start-nodes](./start-nodes-mojo.html}start-nodes) to start a test cluster.  
...
4. Uses [stop-nodes](./stop-nodes-mojo.html) to stop and remove the test 
nodes
